{inputs, username, host, pkgs, lib, ...}: {
  programs.qutebrowser = {
    ...
    # Install Catppuccin theme as an extension
    extraConfig = ''
      # Catppuccin theme setup
      import os
      import sys
      
      # Add the Catppuccin theme directory to the Python path
      config_dir = os.path.dirname(config.configdir)
      sys.path.append(os.path.join(config_dir, "catppuccin"))
      
      try:
          import catppuccin
          # Set the flavor to macchiato (other options: mocha, frappe, latte)
      ...
      # M key for watching videos with mpv
    ...
    settings = {
      ...
      spellcheck.languages = [ "en-US" "uk-UA" "ru-RU" ];
      
      # Tab bar position on the left
      tabs.position = "left";
    };
  };
  
  # Create the catppuccin theme directory
  xdg.configFile = {
    # Catppuccin __init__.py
    "qutebrowser/catppuccin/__init__.py".text = ''
      from .setup import setup
    '';
    
    # Catppuccin setup.py with the theme implementation
    "qutebrowser/catppuccin/setup.py".text = ''
  ...
  # Make sure mpv is available for the M key binding
